...
Ejecute este script desde la consola del instalador de Debian (CTRL + ALT + F2) después de haber particionado y formateado la unidad de destino, y cuando el instalador haya montado el sistema de destino en /target y la partición EFI en /target/boot/efi. Esto es antes del paso de instalación de paquetes.
...
Inicio rápido (descarga con wget) Desde la consola del instalador de Debian (Ctrl+Alt+F2):
...
- Estilo: Bash, set -Eeuo pipefail; salida coloreada con secuencias ANSI e iconos emoji
